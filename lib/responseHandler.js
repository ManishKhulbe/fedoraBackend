...
const {encryptResponse}= require('../lib/middleware/encryptDecrypt');
...
function sendSuccess(response, result,request, shouldEncrypt=false ) {
    var result = new APIResponse(constant.STATUS_CODE.SUCCESS, result,request); 
if(shouldEncrypt){
    result = encryptResponse(result)
}
    _sendResponse(response, result);
}
