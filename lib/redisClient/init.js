...
exports.setOTPValue1 = function (key, value) {
    ...
    // setAsync(key, val , 'EX' , 3*60*60 ) <----- for expiry
    return client.setAsync(keys, val,'EX' , 3*60 ).then(function (response) {
        if (response) {
            // logger.info({'value': response}, '_redisSetValue');
            return true;
        }
...
exports.setSequenceNumber = function(key,value){ 
    let val = JSON.stringify(value)
...
exports.setCommonOTPValue = function (key, value) {
    let val = JSON.stringify(value)
    //set value withou expiry
    let keys = 'ComOTP' + value.params.mobileNo
    // setAsync(key, val , 'EX' , 3*60*60 ) <----- for expiry
    return client.setAsync(keys, val,'EX' , 3*60  ).then(function (response) {
        if (response) {
            // logger.info({'value': response}, '_redisSetValue');
...
exports.setFundTransferOTPValue = function (key, value) {
    let val = JSON.stringify(value)
    //set value withou expiry
    let keys = 'FTOTP' + value.mobileNo
    // setAsync(key, val , 'EX' , 3*60*60 ) <----- for expiry
    return client.setAsync(keys, val,'EX' , 3*60  ).then(function (response) {
        if (response) {
            // logger.info({'value': response}, '_redisSetValue');
...
exports.getComOtpValue = function (key) {
    let keys = 'ComOTP'+ key.mobileNo
...
exports.getSequenceNumber = function (key){
    return client.getAsync(key).then(function (response) {
        console.log("🚀 ~ file: init.js:142 ~ response", response)
...
exports.getFTOtpValue = function (key) {
...
exports.OTPexist = function (key , prefix) {
    console.log(key ,"b")
       let keys = prefix + key
       console.log(keys ,"b")
       return client.existsAsync(keys).then(function (response) {
