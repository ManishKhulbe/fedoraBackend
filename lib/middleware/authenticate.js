    ...
    __verifyTok(acsToken)
        ...
        .then(function (tokenPayload) {
            ...
            if (tokenPayload.d) {
                ...
                if(tokenPayload.d.mobileNo){
                    req.user.mobileNo = tokenPayload.d.mobileNo
                }
...
var autntctTkn1 = function (req, res, next) {
    let acsToken = req.get('accessToken');
    ...
    __verifyTok(acsToken)
        ...
        .then(function (tokenPayload) {
            ...
            if (tokenPayload.d) {
                let userId=mongoose.Types.ObjectId(tokenPayload.d.userId)
                let userMobile = parseInt(tokenPayload.d.mobileNo)
                req.user = tokenPayload.d;
                req.user.userId=userId;
                req.user.userMobile =userMobile
                next()
            } else {
               throw customException.unauthorizeAccess();
            }
