...
const apiLimiter = require("../../../middleware/limitter");
const {decryptRequest} = require('../../../middleware/encryptDecrypt')
...
router
  ...
  .put([middleware.authenticate.autntctTkn], function (req, res) {
    ...
    userFacade
      .updateDeviceToken({ userId, userType, deviceToken })
      .then(function (result) {
        resHndlr.sendSuccess(res, result, req);
      })
      .catch(function (err) {
...
router
  .route("/createMpin")
  .post(
    [middleware.authenticate.autntctTkn, validators.validateMpin],
    function (req, res) {
      console.log(req.body);
      let { mPin } = req.body;
      let { userId, userType } = req.user;
...
router
  ...
  .post(
    ...
    function (req, res) {
      ...
      userFacade
        .userLogin({ mPin, userId, clientIp, mobileNo })
        .then(function (result) {
          resHndlr.sendSuccess(res, result, req);
        })
        .catch(function (err) {
...
router
  ...
  .post(
    ...
    function (req, res) {
      ...
      let clientIp = requestIp.getClientIp(req);
      userFacade
        .validateMPin({ mPin, userId, clientIp, mobileNo })
        .then(function (result) {
          resHndlr.sendSuccess(res, result, req);
        })
        .catch(function (err) {
...
router
  ...
  .post([validators.validateMpin], function (req, res) {
    ...
    let clientIp = requestIp.getClientIp(req);
    userFacade
      .userFirstLogin({ mPin, mobileNo, clientIp })
      .then(function (result) {
        resHndlr.sendSuccess(res, result, req);
      })
      .catch(function (err) {
...
router
  .route("/commonValidateOtp")
  .post([validators.validateForgetPinOtp], function (req, res) {
    let { otp, mobileNo } = req.body;
    userFacade
      .commonValidateOtp({ otp, mobileNo })
      .then(function (result) {
        resHndlr.sendSuccess(res, result, req);
      })
      .catch(function (err) {
...
router
  ...
  .post(
    [
      ...
      middleware.mediaUpload.uploadSingleMediaToS3("profileImage"),
    ],
    function (req, res) {
      let { userId } = req.user;
      ...
      userFacade
        .editProfileImage({ userId, location })
        .then(function (result) {
          resHndlr.sendSuccess(res, result, req);
        })
        .catch(function (err) {
...
router
  .route("/addUser")
  .post(
    [middleware.authenticate.autntctTkn, validators.validateAddUser],
    function (req, res) {
      let { userId, userType } = req.user;
      let {
        membershipId,
      ...
      console.log(membershipId, name, email, password, mPIN, mobileNo);
      userFacade
        .addUser({
          userId,
          membershipId,
...
router
  ...
  .get([middleware.authenticate.autntctTkn], function (req, res) {
    let { userId, userType } = req.user;
    let { status, search, count, pageNo, sortType, sortField, UserType } =
      req.query;
...
router
  .route("/loginAsAdmin")
  .post([apiLimiter , validators.validateLoginAsAdmin , decryptRequest], function (req, res) {
    ...
    userFacade
      ...
      .catch(function (err) {
        resHndlr.sendError(res, err, req , shouldEncrypt);
      });
  });
...
router.route("/validateForgetPassOtp").post(function (req, res) {
  let { otp, email } = req.body;

  userFacade
    .validateForgetPassOtp({ otp, email })
    .then(function (result) {
      resHndlr.sendSuccess(res, result, req);
    })
    .catch(function (err) {
...
router
  ...
  .put([middleware.authenticate.autntctTkn], function (req, res) {
    ...
    let { userId, userType } = req.user;
    userFacade
      .resetPassword({ password, userId, userType })
      .then(function (result) {
        resHndlr.sendSuccess(res, result, req);
      })
      .catch(function (err) {
...
router
  .route("/addOtherBankDetails")
  .post(
    [
      middleware.authenticate.autntctTkn,
      validators.validateAddOtherBankDetails,
    ],
    function (req, res) {
      let { userId, userType } = req.user;
...
router
  ...
  .post([middleware.authenticate.autntctTkn], function (req, res) {
    let { userId, userType } = req.user;
    let { sequenceKeyName , sequenceKeyValue } = req.body;
    userFacade
      .addSequence({
        userId,
        userType,
        sequenceKeyName , sequenceKeyValue
      })
      .then(function (result) {
        resHndlr.sendSuccess(res, result, req);
      })
  ...
  .put([middleware.authenticate.autntctTkn], function (req, res) {
    let { userId, userType } = req.user;
    let { sequenceKeyName , sequenceKeyValue , secretCode} = req.body;
    userFacade
      .editSequence({
        userId,
        userType,
        sequenceKeyName , sequenceKeyValue,secretCode
      })
      .then(function (result) {
        resHndlr.sendSuccess(res, result, req);
      })
