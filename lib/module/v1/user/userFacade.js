...
function getAdminDashBoard(params){
  ...
  return Promise.all([totalNoOfUser,totalNoOfAccount,totalNoOfAccountByType,pendingOnlineServices])
  .then(function (result) {
...
// console.log(result[0]._settledValueField)
    return userMapper.getAdminDashBoardMapping(result[0],result[1],result[2],result[3])
  })
...
function userDetails(params){
  ...
  return usrService.userDetails(params).then(function (data) {
 
    if (data) {
      return userMapper.getUserDetailsMapping(data);
    }else{
      throw customException.noDataFound();
    }
  })
