  ...
  return usrService.isMobileNumberExist(params).then((isExist) => {
    if (isExist) {
      ...
      return redisClient.setCommonOTPValue(otp, userObj).then(function (data) {
        if (data) {
          return appUtils
          .sendSmsToUser(params.mobileNo, `Hello, we have received your request for a new PIN. Your OTP is${self.payload.otp}. Please use this OTP to reset your PIN. This OTP will expire in 3 minutes. Thank you. - FEDORA INDIA `)
            .then((data) => {
...
function forgotPassword(params) {
  ...
  return usrService.isEmailIdExist(params).then(function (isExist) {
    ...
    return redisClient.setOTPValue1(otp, userObj).then(function (data) {
      if (data) {
          let message = `Hello ${isExist.name}, your OTP for resetting your password is: ${otp}. Please enter this OTP within 3 minutes to complete the password reset process. Thank you. - FEDORA INDIA `
          return appUtils
