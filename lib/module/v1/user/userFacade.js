...
const {
  SharedCostPage,
} = require("twilio/lib/rest/api/v2010/account/availablePhoneNumber/sharedCost");
...
function register(params) {
  ...
  return usrService
    ...
    .then(function (data) {
      console.log(data, ">>>>>");
      if (data) {
        // return awsSNS.sendSms( params.mobileNo,
        //   `Dear ${params.name}, your verification OTP for registering on our app is: ${self.payload.otp}. This code will expire in 3 minutes. Thank you for choosing our service. -  FEDORA INDIA`
        // )
        return appUtils
          .sendSmsToUser(
            params.mobileNo,
  ...
  return usrService.isMobileNumberExist(params).then((isExist) => {
    if (isExist) {
      ...
      return redisClient.setCommonOTPValue(otp, userObj).then(function (data) {
        if (data) {
        //   return awsSNS.sendSms( params.mobileNo,
        //     `${self.payload.otp} is your secret One Time Password (OTP). Please do not share it
  ...
  return usrService
    ...
    .then(async function (isNumberExist) {
      ...
      if (params.UserType == 2) {
        ...
        return usrService
          .setSequenceKey("sequenceKey", updatedSequenceKey)
          .then((res) => {
            if (!res) {
        ...
        let updatedSequenceKey = +sequenceKey?.adminSequenceKey + 1;
     
      
        const AdminUserSequence = await usrService.setSequenceKey(
          "adminSequenceKey",
          updatedSequenceKey
        );
        if (!AdminUserSequence) {
          throw customException.unableToUpdateSequence();
        }
        if (params.UserType != 2) {
          if (updatedSequenceKey < 10) {
    ...
    return redisClient.setOTPValue1(otp, userObj).then(function (data) {
      if (data) {
        let message = `Hello ${isExist.name},  ${otp} is your secret One Time Password (OTP). Please do not share it
          with anyone , This OTP will expire in 3 minutes.  FEDORA IN, HO-DIT `;
          // return awsSNS.sendSms( isExist.mobileNo, message)
        return appUtils
          .sendSmsToUser(isExist.mobileNo, message)
          .then(function (data) {
            if (data) {
              return userMapper.otpSentMapping();
            }
...
function editSequenceKeyInRedis(params) {
  return redisClient
    .setSequenceNumber(params.redisKeyName, params.value)
    .then(async (data) => {
      console.log(data);
      if (data) {
        let updatedRedisKey = await redisClient.getSequenceNumber(
          params.redisKeyName
        );
