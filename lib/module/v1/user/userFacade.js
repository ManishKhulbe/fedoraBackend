  ...
  return usrService
    ...
    return usrService.isAccountNumberExists(params ,isExist)
      ...
      .then((isExists)=>{
        ...
        console.log(self.isExist.isRegister,"this.isExist.isRegisterxxxxxxxxxxxxxx")
...
function editUser(params){
  ...
  .then(async function (isExist) {
    ...
    if(params.membershipId){
      let memberShipIdExists = await usrService.isMembershipIdExists(params)
...
function userList(params){
  params.userType = appUtils.isSuperAdminAndAdminAndManagerAndViewer(params.userType);
    ...
    return Promise.all([userList ,totalUserCount ,totalActiveCount ,totalInActiveCount ])
        .then(function (result) {
            return userMapper.userListMapping(result[0],result[1],result[2],result[3],params)
        })
}
...
function logout(params) {
  ...
  return redisSession.expire(params.acsToken)
      .then(function (result) {
  ...
  return redisClient.getForgetPassOtpValue(params).then(function (data) {
    if (data) {
      ...
      if (userObj.otp == params.otp) {
        return usrService.getUserByID(userObj.userId).then(function(userDetails){
...
return redisSession.create(userObj).then(function (redisToken) {
  ...
  return userMapper.validateForgetPassOtpMapping(userObj , redisToken);
})
...
function getAdminDashBoard(params){
  ...
  const pendingOnlineServicesByType = usrService.pendingOnlineServicesByType(params)

  return Promise.all([totalNoOfUser,totalNoOfAccount,totalNoOfAccountByType,pendingOnlineServices , pendingOnlineServicesByType])
  .then(function (result) {
...
// console.log(result[0]._settledValueField)
    return userMapper.getAdminDashBoardMapping(result[0],result[1],result[2],result[3], result[4])
  })
...
const totalLoan = usrService.totalLoan(params)
  ...
  return Promise.all([totalDeposit , totalLoan])
  .then(function (result) {
...
async function deleteOtherBankDetails(params){
  console.log("🚀 ~ file: userFacade.js:948 ~ deleteOtherBankDetails ~ params", params)
  params.userType = appUtils.isAdmin(params.userType);
