...
function register(params) {
  ...
  return usrService
    ...
    .then(function (data) {
      ...
      if (data) {
        return appUtils
          .sendSmsToUser(params.mobileNo, `Dear ${params.name}, your verification OTP for registering on our app is: ${self.payload.otp}. This code will expire in 3 minutes. Thank you for choosing our service. Team FEDORA INDIA`)
          .then((data) => {
            if (data) {
  ...
  return usrService.isMobileNumberExist(params).then((isExist) => {
    if (isExist) {
      ...
      return redisClient.setCommonOTPValue(otp, userObj).then(function (data) {
        if (data) {
          return appUtils
          .sendSmsToUser(params.mobileNo, `Hello, we have received your request for a new PIN. Your OTP is${self.payload.otp}. Please use this OTP to reset your PIN. This OTP will expire in 3 minutes. Thank you."  `)
            .then((data) => {
  ...
  return usrService
    ...
    .then(async function (isNumberExist) {
      ...
      if (params.UserType == 2) {
        let sequenceKey = await redisClient.getSequenceNumber("sequenceKey");
        console.log(sequenceKey, "sequenceKeysequenceKey");
        let sequenceKeyInRedis = +sequenceKey + 1;
        return redisClient
          .setSequenceNumber("sequenceKey", sequenceKeyInRedis)
          .then((res) => {
            if (params.UserType == 2) {
              if (sequenceKeyInRedis < 10) {
                params.membershipId = `FED-000${sequenceKeyInRedis}`;
              } else if (sequenceKeyInRedis < 100) {
                params.membershipId = `FED-00${sequenceKeyInRedis}`;
              } else if (sequenceKeyInRedis < 1000) {
                params.membershipId = `FED-0${sequenceKeyInRedis}`;
              } else {
                params.membershipId = `FED-${sequenceKeyInRedis}`;
              }
            }
            if (res) {
...
function forgotPassword(params) {
  ...
  return usrService.isEmailIdExist(params).then(function (isExist) {
    ...
    return redisClient.setOTPValue1(otp, userObj).then(function (data) {
      if (data) {
          let message = `Hi ${isExist.name} , your 4 digit otp for forgotPassword is ${otp} ,Please don't share otp with anyone `
          return appUtils
