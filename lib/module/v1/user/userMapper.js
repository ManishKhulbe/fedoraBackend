...
function userListMapping(params,totalUserCount){
  var respObj = {
    message: "user list",
    result: params,
    totalUserCount
...
function getAdminDashBoardMapping(totalNoOfUser,totalNoOfAccount,totalNoOfAccountByType,pendingOnlineServices){
  
  var respObj = {
    message: "fetched successfully",
    totalNoOfUser,totalNoOfAccount,totalNoOfAccountByType,pendingOnlineServices
  };
  return respObj;
}

function getUserDetailsMapping(params){
  let date = new Date(params.updatedAt);
  let year = date.getFullYear();
  let month = date.getMonth() + 1;
  let dt = date.getDate();
