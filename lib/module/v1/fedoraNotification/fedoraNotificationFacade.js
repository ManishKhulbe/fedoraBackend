...
function list(params) {
  ...
  return Promise.all([getNotification])
  .then(function (result) {
    return fedoraNotificationMapper.getNotificationMapper(
      result[0],
      params
    );
  });
...
function notificationDetails(params) {
    // console.log(params)
    // params.userType = appUtils.isAdmin(params.userType);
  const getNotification = fedoraNotificationService.getNotificationDetails(params);
...
crons.deleteNotification(cronString)
}

async function getNotificationDeleteByMonth(params) {
   
    const getNotification = await fedoraNotificationService.getNotificationDeleteByMonth(params);
}
...
module.exports = {
   list,notificationDetails,deleteNotificationCron,getNotificationDeleteByMonth
}
