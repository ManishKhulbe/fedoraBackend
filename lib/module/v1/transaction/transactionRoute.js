...
router.route('/listByUser')
    ...
    .get([middleware.authenticate.autntctTkn], function (req, res) {
        ...
        transactionFacade.listByAdmin({senderAccountType ,receiverAccountType ,userId,status ,userType, search,searchDate,searchYear,searchMonth,searchDay,sortType,sortField ,pageNo ,count,startDate,endDate})
            .then(function (result) {
                resHndlr.sendSuccess(res, result, req)
            }).catch(function (err) {
            resHndlr.sendError(res, err, req)
