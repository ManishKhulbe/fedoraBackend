...
async function editTransaction(params) {
  ...
  try {
    ...
    //update sender account balance in transaction 

    let query2 = {};
    query2._id = editTransaction._id;

    let editSenderBalanceTransaction = await transactionDao
      .findOneAndUpdate(
        query2,
        { $set: { senderAccountBalance: +senderAccountUpdate.balance } },
        { new: true, session: session }
      )
      .then(function (result) {
        if (result) {
    ...
    if (!editSenderBalanceTransaction) {
      throw customException.failedToDebit(params);
    }
    }
    ...
    return transactionMapper.editOtherBankTransactionMapping(
      editTransaction
    );
