...
function initiateTransaction(senderAccount ,receiverAccount , params ,status){
    // console.log(",,,,,,,,,,,,",senderAccount ,receiverAccount , params ,status)
    let newTransaction = {}
    ...
    return transactionDao.save(newTransaction).then((res)=>{
        return res
    })
    .catch((err)=>{
        return false
...
function initiateTransactionByAdmin(params ,result){
    console.log("🚀 ~ file: transactionDao.js:100 ~ initiateTransactionByAdmin ~ params ,result", params ,result)
    // console.log(",,,,,,,,,,,,",senderAccount ,receiverAccount , params ,status)
    let newTransaction = {}
    ...
    newTransaction.remark = params.remark
    newTransaction.createdAt = params.date
...
function getAllViewTransactionsAndFilter(params){
    ...
    if(params.accountNumber ){
        aggPipe.push({
            $match: { $or: [ { accountNumberFrom: params.accountNumber }, { accountNumberTo:params.accountNumber  } ] }
        })
...
//     if (params.search && !appUtils.isInteger(params.search)) {
  ...
  function detailsById(params){
    ...
    return transactionView.find({_id : appUtils.objectIdConvert(params.transactionId) }).toArray().then((result)=>{
        if(result.length != 0){
            return result[0]
        }else{
            return false
  ...
  function editTransaction(params , session){
    // console.log("🚀 ~ file: transactionDao.js:821 ~ editTransaction ~ session", session)
    return transactionDao.findOneAndUpdate({_id : appUtils.objectIdConvert(params.transactionId)},{$set : {status : +params.status}} ,{ new: true, session: session }).then((result)=>{
        if(result){
            return result
        }else{
            return false
