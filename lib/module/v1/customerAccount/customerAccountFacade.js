...
async function initiateFundTransfer(params) {
  ...
  return userService.isUserIdExist(params).then(async (isExist) => {
    if (isExist) {
      ...
      let estimatedBalanceAfterTxn =
        getSenderAccountDetails[0].balance - +params.transactionAmount;

      if (
        getSenderAccountDetails[0].accountType == 1 &&
        estimatedBalanceAfterTxn <= (process.env.MINIMUM_BALANCE || 2000)
      ) {
        throw customException.setMinBalanceLimitExceeded(
          process.env.MINIMUM_BALANCE
        );
      }
...
async function fundTransferToOtherBank(params) {
  ...
  if (senderAccount.accountType == 4 || senderAccount.accountType == 5) {
    ...
    if (estimatedBalanceAfterTxn <= process.env.MINIMUM_BALANCE) {
      throw customException.setMinBalanceLimitExceeded(
        process.env.MINIMUM_BALANCE
      );
    }
  ...
  for (let i = 0; i < receiverAccount.otherBankDetails.length; i++) {
    if (receiverAccount.otherBankDetails[i]._id == params.otherBankDetailsId) {
      receiverOtherBankDetails = receiverAccount.otherBankDetails[i];
      break;
    }
  }
  console.log(receiverOtherBankDetails, "receiverOtherBankDetails");
  ...
  try {
    let estimatedBalanceAfterTxn =
      senderAccount.balance - +params.transactionAmount;
    console.log(
      "🚀 ~ file: customerAccountFacade.js:625 ~ fundTransferToOtherBank ~ estimatedBalanceAfterTxn:",
      estimatedBalanceAfterTxn
    );
    const updateSenderAccountBalance =
      await customerAccountDao.findOneAndUpdate(
        { _id: senderAccount._id },
        { $set: { balance: estimatedBalanceAfterTxn } },
        { new: true, session }
      );
 

    const initiateTransaction =
      await customerAccountService.initiateTransaction(
        updateSenderAccountBalance,
        receiverOtherBankDetails,
        params,
        1
      );
    console.log(
      "🚀 ~ file: customerAccountFacade.js:627 ~ fundTransferToOtherBank ~ updateSenderAccountBalance:",
      updateSenderAccountBalance,
      "🚀 ~ file: customerAccountFacade.js:638 ~ fundTransferToOtherBank ~ initiateTransaction:",
      initiateTransaction
    );
