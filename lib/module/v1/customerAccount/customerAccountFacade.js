...
function create(params) {
  ...
  return customerAccountService
    .sameAccountNumberExists(params)
...
async function initiateFundTransfer(params) {
  ...
  let self = this;
  return userService.isUserIdExist(params).then((isExist) => {
    if (isExist) {
      console.log(isExist);
    ...
    if (userDetails) {
      return redisClient
        ...
        .then((isExist) => {
          ...
          if (isExist) {
            return redisClient.getFTOtpValue(userDetails.mobileNo).then((data) => {
              let finalData = JSON.parse(data);
              console.log(finalData, "finalUserDatafinalUserDatafinalUserData");
