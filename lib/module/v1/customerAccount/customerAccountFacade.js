...
for(let i = 0 ; i< deviceIds.length ; i++){
  if(i == 0){
    notificationDao.addNotification({customerId : senderDeviceToken._id , profileImg : receiverDeviceToken?.profileImage,title : receiverDeviceToken.name , subTitle : `${result?.finalTransaction?.transactionAmount} sent` , body : dataToUser[i] ,  additional : result.finalTransaction })
    pushNotification.pushNotification(deviceIds[i] , dataToUser[i] , "Debit Successfully")
  }else{
    notificationDao.addNotification({ customerId : receiverDeviceToken._id , profileImg  : senderDeviceToken?.profileImage, title : senderDeviceToken.name , subTitle : `${result?.finalTransaction?.transactionAmount} received` , body : dataToUser[i] ,  additional : result.finalTransaction })
