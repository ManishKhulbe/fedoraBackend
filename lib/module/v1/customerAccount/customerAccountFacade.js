...
async function create(params) {
  ...
  return customerAccountService
    ...
    .then(async (isExist) => {
      ...
      console.log("🚀 ~ file: customerAccountFacade.js:110 ~ .then ~ accountNumberSeqKeyInMOngo", accountNumberSeqKeyInMOngo)
      return customerAccountService
...
eventEmitter.on('sendNotification', async (result, params , insetTransaction)=>{
  ...
  if(params.transactionType == 1){
    message = `Your A/c ******${result.accountNumber.slice(-4)} is Credited for Rs:${(+params.amount).toFixed(2)} on ${(result.updatedAt).toLocaleString('en-IN', { timeZone: 'Asia/Kolkata' })} by ${params.remark? params.remark : "" }  Avl Bal Rs:${(+result.balance).toFixed(2)} - Fedora India`
  }else{
    message=`Your a/c no. ******${result.accountNumber.slice(-4)} is debited for Rs.${(+params.amount).toFixed(2)} on ${(result.updatedAt).toLocaleString('en-IN', { timeZone: 'Asia/Kolkata' })} By ${params.remark? params.remark : "" } Avl Bal Rs:${(+result.balance).toFixed(2)} - Fedora India`
  }
