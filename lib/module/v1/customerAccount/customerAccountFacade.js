...
async function create(params) {
  ...
  return customerAccountService
    ...
    .then(async (isExist) => {
      ...
      if(isExist.length > 2 && (parseInt(accountDetails.accountType) ==7 || parseInt(accountDetails.accountType) ==4 || parseInt(accountDetails.accountType) ==2 || parseInt(accountDetails.accountType)==6)){
        throw customException.sameCustomerAlreadyExistsAccountType();
      }else if(isExist.length > 4 && parseInt(accountDetails.accountType) ==3){
        throw customException.sameCustomerAlreadyExistsAccountType();
      } else if(isExist.length != 0  && (parseInt(accountDetails.accountType) == 1 || parseInt(accountDetails.accountType) ==5)){
        throw customException.sameCustomerAlreadyExistsAccountType();
      }
      ...
      switch (self.accountDetails.accountType) {
        case 1:
          accountNumberSeqKey = await redisClient.getSequenceNumber(
            "accountNumberSeqKey1"
          );
          keyInRedis = "accountNumberSeqKey1";
          break;
        case 2:
          accountNumberSeqKey = await redisClient.getSequenceNumber(
            "accountNumberSeqKey2"
          );
          keyInRedis = "accountNumberSeqKey2";
          break;
        case 3:
          accountNumberSeqKey = await redisClient.getSequenceNumber(
            "accountNumberSeqKey3"
          );
          keyInRedis = "accountNumberSeqKey3";
          break;
        case 4:
          accountNumberSeqKey = await redisClient.getSequenceNumber(
            "accountNumberSeqKey4"
          );
          keyInRedis = "accountNumberSeqKey4";
          break;
        case 5:
          accountNumberSeqKey = await redisClient.getSequenceNumber(
            "accountNumberSeqKey5"
          );
          keyInRedis = "accountNumberSeqKey5";
          break;
        case 6:
          accountNumberSeqKey = await redisClient.getSequenceNumber(
            "accountNumberSeqKey6"
          );
          keyInRedis = "accountNumberSeqKey6";
          break;
        case 7:
          accountNumberSeqKey = await redisClient.getSequenceNumber(
            "accountNumberSeqKey7"
          );
          keyInRedis = "accountNumberSeqKey7";
          break;
      }

      console.log(
        "🚀 ~ file: customerAccountFacade.js:65 ~ .then ~ accountNumberSeqKey",
        accountNumberSeqKey
      );

      let accountNumberSeqKeyInRedis = +accountNumberSeqKey + 1;
      return redisClient
        .setSequenceNumber(keyInRedis, accountNumberSeqKeyInRedis)
        .then((res) => {
          if (res) {
...
async function editAccount(params){
  params.userType = appUtils.isAdmin(
    params.userType
  );
    ...
    if (isExist) {
      ...
      return redisClient
        ...
        .then(function (data) {
          if (data) {
            return appUtils
              .sendSmsToUser(isExist.mobileNo, `Hello ${isExist.name}, your verification OTP for the fund transfer is ${self.payload.otp}. Please enter this code to complete your transaction. Thank you for using our service. - FEDORA INDIA`)
              .then((data) => {
...
async function fundTransfer1(params) {
  ...
  return fundTransferLogic
    ...
    .then(async (result) => {
      ...
      let dataToUser = [];
      let SenderMsgStr = ` Your a/c no. ****${(result?.senderAccountUpdate?.accountNumber).toString().slice(-4)} is debited for Rs.${result?.finalTransaction?.transactionAmount} on ${result?.finalTransaction?.createdAt} By(Cash/Transfer) Avl Bal Rs:${
      ...
      let receiverMsgStr = `Your A/c ${(result?.receiverAccountUpdate?.accountNumber)
        ...
        Remark field- By Cash/By Transfer)ref no ${result?.finalTransaction?._id} Avl Bal Rs:${result?.receiverAccountUpdate.balance} -
        Fedora India`;
    ...
    .then(async function (result) {
      if (result) {
        ...
        .then((insetTransaction) => {
            eventEmitter.emit('sendNotification', result, params,insetTransaction )
...
eventEmitter.on('sendNotification', async (result, params , insetTransaction)=>{
  ...
  if(params.transactionType == 1){
    message = `Your A/c ******${result.accountNumber.slice(-4)} is Credited for Rs:${(+params.amount).toFixed(2)} on ${result.updatedAt} by ( Remark field- By Cash/By Transfer) ref no ${insetTransaction._id} Avl Bal Rs:${(+result.balance).toFixed(2)} - Fedora India`
  }else{
    message=`Your a/c no. ******${result.accountNumber.slice(-4)} is debited for Rs.${(+params.amount).toFixed(2)} on ${result.updatedAt} By (Cash/Transfer) Avl Bal Rs:${(+result.balance).toFixed(2)} - Fedora India`
  }
