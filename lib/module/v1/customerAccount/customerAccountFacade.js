...
// if(isMainThread){
  ...
  return fundTransferLogic.fundTransfer( params).then(async (result)=>{
   ...
   let deviceIds = [senderDeviceToken , receiverDeviceToken  ]
   let dataToUser = []
let SenderMsgStr = `Debit
INR ${result.finalTransaction.transactionAmount} 
A/c no. ${result.senderAccountUpdate.accountNumber}, 
${result.transactionDetails.createdAt}
your remaining account balance is ${result.senderAccountUpdate.balance} - Fedora MBanking`
let receiverMsgStr = `Dear Customer An Amount of ${result.finalTransaction.transactionAmount} is credited to your account Number ${result.receiverAccountUpdate.accountNumber}, from ${result.senderAccountUpdate.accountNumber} `

   dataToUser.push(SenderMsgStr)
  dataToUser.push(receiverMsgStr)

    // pushNotification.pushNotification(deviceIds , dataToUser)
