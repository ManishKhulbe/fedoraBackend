...
function create(params) {
  ...
  return customerAccountService.sameAccountNumberExists(params).then(async function (isExist){
    if(isExist.length != 0){
    throw customException.accountNumberAlreadyExist();
    }
    const accountDetails = await accountDao.isAccountIdExists(params)
    self.accountDetails = accountDetails
    return customerAccountService.sameTypeAccountExists(params , accountDetails).then((isExist)=>{
...
// if(isMainThread){
  ...
  return fundTransferLogic.fundTransfer( params).then(async (result)=>{
  
    const senderDeviceToken = await  userDao.getUserDeviceToken(result.senderAccountUpdate.customerId)
    const receiverDeviceToken = await  userDao.getUserDeviceToken(result.receiverAccountUpdate.customerId)
  
   let deviceIds = [senderDeviceToken.deviceToken , receiverDeviceToken.deviceToken ]
   let dataToUser = []
...
your remaining account balance is ${result?.senderAccountUpdate?.balance} - Fedora MBanking`



let receiverMsgStr = `Dear Customer An Amount of INR
${result?.finalTransaction?.transactionAmount} is credited to your account Number 
********${(result?.receiverAccountUpdate?.accountNumber).toString().slice(-4)}, from 
...
for(let i = 0 ; i< deviceIds.length ; i++){
  if(i == 0){
    notificationDao.addNotification({customerId : senderDeviceToken._id , profileImg : receiverDeviceToken?.profileImage,title : receiverDeviceToken.name , subTitle : `${result?.finalTransaction?.transactionAmount} sent` , body : dataToUser[i] ,  additional : JSON.stringify(result.finalTransaction)  })
    pushNotification.pushNotification(deviceIds[i] , dataToUser[i] , "Debit Successfully")
  }else{
    notificationDao.addNotification({ customerId : receiverDeviceToken._id , profileImg  : senderDeviceToken?.profileImage, title : senderDeviceToken.name , subTitle : `${result?.finalTransaction?.transactionAmount} received` , body : dataToUser[i] ,  additional : JSON.stringify(result.finalTransaction) })
    pushNotification.pushNotification(deviceIds[i] , dataToUser[i] ,  "Credit Successfully")
  }
}
...
async function fundTransferToOtherBank(params){
  ...
  for(let i = 0 ; i< receiverAccount.otherBankDetails.length ; i++){
    ...
    if(receiverAccount.otherBankDetails[i]._id == params.otherBankDetailsId){
      ...
      return customerAccountService.initiateTransaction(senderAccount ,receiverAccount.otherBankDetails[i] , params ,1).then(async function(transactionDetails){
        if(transactionDetails){
...
// params.userType = appUtils.isAdmin(params.userType);
 ...
 function addBalance(params) {
  ...
  return customerAccountService.addBalance(params).then(async function (result) {
    if (result) {
      return transactionDao.initiateTransactionByAdmin(
        params , result
      ).then((insetTransaction)=>{
      return customerAccountMapper.addBalanceMapping(result , insetTransaction);
      })
    } else {
      throw customException.receiverAccountNumberNotExists();
    }
