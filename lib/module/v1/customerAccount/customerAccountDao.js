...
function create(params, accountDetails) {
  ...
  if(params.allotedAmount){
    params.allotedAmount = parseFloat((+params.allotedAmount).toFixed(2));
  }
  if(params.rdAmount){
    params.rdAmount =  parseFloat((+params.rdAmount).toFixed(2));
  }
...
update.accountInterestRate = parseFloat(params.accountInterestRate)
  }
  if(params.allotedAmount){
    update.allotedAmount = parseFloat((+params.allotedAmount).toFixed(2));
  }
  if(params.tenure){
    update.tenure = +params.tenure
  }
  if(params.rdAmount){
    update.rdAmount = parseFloat((+params.rdAmount).toFixed(2));
  }

  return customerAccountDao.findOneAndUpdate(query , update ,{new : true}).then((res)=>{
...
async function getAllAccountAndFilter(params) {
  ...
  if (params.accountStatus) {
    aggPipe.push({ $match: { accountStatus: parseInt(params.accountStatus) } });
  }
  if (params.AccountType) {
  ...
  return await customerAccountDao.aggregate(aggPipe).then((res) => {
    // console.log(res[0].pipelineResults, "vvv");
    if (res.length != 0) {
      return res;
    } else {
      return false;
...
function getUserAccount(params) {
  ...
  aggPipe.push({
    $addFields: {
      ...
      //tenure is comes in month
      rdTotalAmount: {
        $multiply : ["$balance", "$tenure"]
      },
    },
  });
  ...
  aggPipe.push({
    $addFields: {
      maturityAmount: {
        ...
        then:{
          $round: [ { $add: ["$rdTotalAmount", { $multiply: ["$rdTotalAmount", "$secondPart"] }] }, 2 ],
        },
        else:{
          $round: [
...
async function addBalance(params) {
  ...
  if (params.transactionType == 1) {
    update.balance = parseFloat((+params.amount).toFixed(2));
  } else {
    update.balance = -parseFloat((+params.amount).toFixed(2));
  }
...
function totalNoOfAccountByType(params) {
  ...
  aggPipe.push({
    $group: {
      ...
      totalLoanAmount : {$sum : '$allotedAmount'}
    },
  });

  //total amount fix to 2 decimal
  aggPipe.push({
    $addFields: {
      totalAmount: {
        $round: ["$totalAmount", 2],
