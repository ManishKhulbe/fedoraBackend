...
router.route('/create')
    .post([middleware.authenticate.autntctTkn,validators.validateCreateService ,middleware.multer.single('serviceLogo'), middleware.mediaUpload.uploadSingleMediaToS3('serviceLogo') ],function (req, res) {
        
        let {userId, userType} = req.user
        let {serviceType,location  ,discription } = req.body
      
        serviceFacade.create({serviceType,discription ,location  ,userId, userType})
