...
router.route('/create')
    ...
    .put([middleware.authenticate.autntctTkn ,middleware.multer.single('optionLogo'), middleware.mediaUpload.uploadSingleMediaToS3('optionLogo')],
    function (req, res) {
        console.log(req.body ,"VVVVVVV")
        let {userId, userType} = req.user
        let {serviceId,optionId   ,optionName  , optionAction ,optionType , location} = req.body
 
        serviceFacade.editOptionAction({location,serviceId,optionId   ,optionName ,optionType , optionAction ,userId, userType})
            .then(function (result) {
                resHndlr.sendSuccess(res, result, req)
            }).catch(function (err) {
            resHndlr.sendError(res, err, req)
