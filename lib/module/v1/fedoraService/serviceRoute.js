...
router.route('/create')
    ...
    function (req, res) {
        console.log(req.body ,"VVVVVVV")
        let {userId, userType} = req.user
        let {optionName,location   ,serviceId  , optionAction } = req.body

        serviceFacade.createOptions({optionName  ,location  ,userId, userType ,serviceId  , optionAction})
            .then(function (result) {
                resHndlr.sendSuccess(res, result, req)
            }).catch(function (err) {
            resHndlr.sendError(res, err, req)
    ...
    function (req, res) {
        console.log(req.body ,"VVVVVVV")
        let {userId, userType} = req.user
        let {serviceId,optionId   ,optionName  , optionAction } = req.query

        serviceFacade.editOptionAction({serviceId,optionId   ,optionName  , optionAction ,userId, userType})
            .then(function (result) {
                resHndlr.sendSuccess(res, result, req)
            }).catch(function (err) {
            resHndlr.sendError(res, err, req)
