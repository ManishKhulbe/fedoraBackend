...
function editByCustomer(params ,serviceDetails){
  ...
  if(serviceDetails[0].serviceType != 3){
    update.orderDate = params.orderDate
    update.timeSlot = params.timeSlot
  }
  // if(serviceDetails[0].serviceType == 2){
  //   orderDate
  //   timeSlot
  // }
  ...
  update.remark = params.remark?params.remark:"Rejected/edit by customer"
...
function isOnlineServiceIdExists(params){
  return onlineServiceDao.find({_id   : apputils.objectIdConvert(params.onlineServiceId) , customerId : apputils.objectIdConvert(params.userId)}).then((res) => {
    // console.log(res ,"..")
    if (res) {
      return res;
    } else {
...
function getCustomerRequestHistory(params){
  console.log(params)
  ...
  if(params.optionActionType){
    aggPipe.push({
      $match : {
        optionActionType : parseInt(params.optionActionType)
      }
    })
  }
