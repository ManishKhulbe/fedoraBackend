...
async function editByCustomer(params){
  const serviceDetails = await onlineServiceService.isOnlineServiceIdExists(params)
  if(!serviceDetails){
    throw customException.onlineServiceIdNotExists(); 
  }
  console.log(serviceDetails,"cccc")
  
  return onlineServiceService.editByCustomer(params ,serviceDetails)
.then(function (account) {
  if(account){
    return onlineServiceMapper.editByCustomerMapping(account);
  }else{
    throw customException.onlineServiceIdActionNotApplicable(); 
  }
}
)
.catch(function (err) {
...
function listByCustomer(params){

  const getCustomerRequestHistory =
  onlineServiceService.getCustomerRequestHistory(params);

  return Promise.all([getCustomerRequestHistory])
  .then(function (result) {
    return onlineServiceMapper.getCustomerRequestHistoryMapping(
      result[0],
      params
    );
  });
...
module.exports = {
  editByCustomer,  apply,list,edit,deleteService,applyFedoraCash,listByCustomer
};
